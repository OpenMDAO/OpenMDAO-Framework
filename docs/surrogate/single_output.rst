...
Once the MetaModel component is in place, the first step is to fill the `default_surrogate` slot. 
...
Once the `default_surrogate` and `model` slots of the MetaModel have been filled, the MetaModel
