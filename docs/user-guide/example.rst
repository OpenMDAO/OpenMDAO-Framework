...
constructors. The Public Variable constructors require the first two inputs but also allow you to specify several
optional parameters.
...
should be more efficient, although for simple calculations like this, the difference would not be noticeable. The
...
connecting their inputs and outputs. The data connections define an execution order based on their dependencies, i.e.,
components that are upstream in the data flow will be executed prior to those downstream so that input data to a
component will always be valid with respect to the other parts of the workflow. Component execution is also lazy,
meaning that a component will not execute if its inputs have not changed since its last execution. In addition, an
Assembly can also contain a Driver, such as an optimizer or a design study. When an Assembly does not
explicitly contain a Driver, the assembly executes the components based on the data connections.
...
Now that the components are instantiated in the Assembly, we need to hook them up:
...
for the Vehicle class. In the future, you might want to replace the current vehicle model with a new model. This
new model will be compatible provided that it has the same interface as the current vehicle model. The interface
checking is  facilitated by creating a Socket in the vehicle simulation assembly.
...
The final step is to create a top level Assembly which defines the problem using DrivingSim and the vehicle Assembly. The first
problem we would like to solve is a single objective optimization problem where we adjust some subset of the design variables
to minimize the 0-60 acceleration time. We chose the design variables *bore* and *spark angle*; the optimal value of the
first variable should be quite intuitive (i.e., larger bore means faster acceleration), but the second variable cannot be
optimized by mere inspection. 
