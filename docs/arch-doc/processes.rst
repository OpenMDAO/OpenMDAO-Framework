...
:term:`Assembly` is a Component that contains other Components along with an object
...
order specified by the :term:`Workflow` object.  A :term:`Driver` is a
Component that performs some kind of iteration over a :term:`Workflow`,
iterating until some condition is met. A :term:`Driver` that is an optimizer,
for example, would iterate over a :term:`Workflow` until it satisfies some
...
:term:`CaseIterator`, will iterate over a :term:`Workflow` until it uses up
