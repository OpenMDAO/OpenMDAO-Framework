...
def write(name, doc, version, entry_map, src_files, distributions, modules,
...
""" % {'egg_name':egg_name, 'py_version':sys.version[:3]}
    ...
    pkg_info.append('Summary: %s' % doc.split('\n')[0])
    pkg_info.append('Description: %s' % doc)
    ...
    _write_info(egg, 'openmdao_orphans.txt', orphans, logger)
...
def write_via_setuptools(name, doc, version, entry_map, src_files,
                         distributions, modules, dst_dir, logger):
