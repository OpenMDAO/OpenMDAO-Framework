...
def _addPrefixed(unit):
  ...
  for prefix in prefixes:
    name = prefix[0] + unit
    addUnit(name, prefix[1]*unit_table[unit])
...
def importLibrary(libfile):
  _unitLib.read(libfile)
...
importLibrary("unitLibdefualt.ini")
...
if __name__ == "__main__":
  ...
  length1 = PhysicalQuantity('6m')
  length2 = PhysicalQuantity(10,'cm')

  print length1+length2
  print (length1+length2).inUnitsOf('cm')
  print (length1+length2).inUnitsOf('dam')




func='math.cos(x*y)*math.sin(x**2)+math.exp(x)'
func1='x1**2+2*x1+2'
func2='math.sqrt(1-x1^2+x2^2+x3^2)'
func21='x'
func3='math.cos(x)'
func11='((x1+0.001)**2+2*(x1+0.001)+2)-((x1-0.001)**2+2*(x1-0.001)+2)/(2*0.001)'

class F(object):
    def __init__(self,string_func):
        self.origstr=string_func
        self.mstr=string_func
        self.str2=self.origstr
        self.n = re.findall('(?<=)\w+', string_func)
        self.u=dict(zip(self.n,self.n)).keys()
        ...
        while i!=elements:
            ...
            if self.u[i]=='1e':
                ...
                i=i-1
            if self.u[i].isdigit():
                self.u.remove(self.u[i])
                elements=elements-1
                i=i-1
            if math.__dict__.has_key(self.u[i]):
                self.u.remove(self.u[i])
                elements=elements-1
                i=i-1
            i=i+1
        self.u=sorted(self.u)
        self.numVar=len(self.u)
    def evalf(self,*pts):

        for i in range(0,self.numVar):
            self.mstr=self.mstr.replace(self.u[i],str(pts[i]))
        return eval(self.mstr)
    def diff(self,var,h,*pts):
       deriv=str('(('+self.origstr.replace(var,'('+var+'+'+str(h)+')')+')-'+'('+self.origstr.replace(var,'('+var+'-'+str(h)+')')+'))/(2*'+str(h)+')')
       nderiv=F(deriv)
       print len(pts)
       print nderiv.evalf(1)
