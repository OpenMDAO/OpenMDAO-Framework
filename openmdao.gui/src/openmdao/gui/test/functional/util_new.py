...
from pageobjects.project_new import ProjectsPage
from pageobjects.util import SafeDriver, abort

from pageobjects.workspace_new import WorkspacePage
...
def new_project(new_project_modal, verify=False, load_workspace=False):
    ...
    if verify:
        data = dict(name=new_project_modal.get_random_project_name(),
                    ...
                    version='12345678')
        page = submit_metadata(new_project_modal, data['name'], data['description'], data['version'], load_workspace)

    else:
        data = dict(name=new_project_modal.get_random_project_name())
        page = submit_metadata(new_project_modal, data['name'], verify=verify, load_workspace=load_workspace)
...
def import_project(import_project_modal, name, description=None, version=None, verify=False, load_workspace=False):
    """
    Creates a randomly-named new project.
    Returns ``(projects_page, info_dict)``
    """
    assert import_project_modal.modal_title.startswith('Import Project')
    return submit_metadata(import_project_modal, name, descript, version, load_workspace)
