...
def setup_server() :
    ...
    server = Process(target=_run, args=('--server', '--port', str(port)))
    server.start()
    logging.critical('server started')
...
def teardown_server():
    ...
    for browser in browsers:
        browser.close()
    server.terminate()
    logging.critical('server terminated')
    print >>sys.stderr, 'server terminated'
