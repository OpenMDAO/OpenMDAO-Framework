...
class WorkspacePageObject(BasePageObject):
    ...
    def __init__(self, browser, port):
        ...
        self.locators["objects_tab"] = ( By.XPATH, '//*[@id="otree_tab"]' )
        ...
        self.locators["new_file_menu_item"] = ( By.XPATH, '/html/body/div/dl/dd[2]/div/nav2/ul/li/ul/li[1]/a' ) 
        ...
        self.locators["edit_file"] = ( By.XPATH, "//a[(@rel='editFile')]" )
        self.locators["show_structure"] = ( By.XPATH, "//a[(@rel='show_dataflow')]" )
        ...
        self.locators["specify_name_input"] = ( By.XPATH, "/html/body/div[12]/div[2]/input" )

        #self.locators["code_input"] = ( By.XPATH, "//div[@class='CodeMirror-wrapping']" )
    ...
    def objects_tab( self ):
        WebDriverWait(self.browser,10).until(
            lambda browser:self.browser.find_element(*self.locators["objects_tab"]) ).click()
    ...
    def get_objects_attribute( self, attribute  ):
        ...
        for o in object_elements:
            object_attributes.append( o.get_attribute( attribute ) )
    ...
    def new_file( self, filename, code ):
        ...
        # Click New File menu item
        time.sleep(3)
        WebDriverWait(self.browser,10).until(
            lambda browser:self.browser.find_element(*self.locators["new_file_menu_item"]) ).click()

        # popup div is displayed
        name_input_element = WebDriverWait(self.browser,10).until(
            lambda browser:self.browser.find_element(*self.locators["specify_name_input"]) )
        name_input_element.send_keys(filename)
        name_input_element.send_keys(Keys.RETURN)

        self.edit_file( filename )
        # The editing area is in an iframe so I need to switch to it using
        self.browser.switch_to_frame("code-editor")
        # Remember the locator is relative to the iframe not the top level window
        code_input_element = WebDriverWait(self.browser,10).until(
            lambda browser:self.browser.find_element(*self.locators["code_input"]) )
        ...
        # Control-S to save. Need to make sure that the Code div has the focus
        #   otherwise the control S goes to the browser and it tries to do a save
        action_chains = ActionChains(self.browser)
        action_chains.click( code_input_element ).key_down( Keys.CONTROL ).send_keys( "s" ).key_up( Keys.CONTROL ).perform()
    ...
    def edit_file(self, filename) :
        '''Context click and select edit file, or
        ...
        #action_chains.context_click( f ).perform()
        #self.browser.find_element(*self.locators["edit_file"]).click()

    def show_structure(self, component_name) :
        f = self.browser.find_element_by_xpath("//div[@id='otree']//li[(@path='%s')]" % component_name )
        ...
        WebDriverWait(self.browser,10).until(
            lambda browser:self.browser.find_element(*self.locators["show_structure"]) ).click()
...
class ProjectsListPageObject(BasePageObject):
    ...
    def logout(self):
        ...
        #self.browser.find_element(*self.locators["logout"]).click()
        WebDriverWait(self.browser, 10).until(lambda s: self.browser.title.startswith('Login'))
        return LoginPageObject( self.browser, self.port )
...
class LoginPageObject(BasePageObject):
    ...
    def __init__(self, browser, port):
        ...
        print >>sys.stderr, 'LoginPageObject'
...
#        self._page_url = "http://localhost:%d/accounts/login/?next=/" % self.port
        ...
        print >>sys.stderr, 'LoginPageObject', self._page_url
