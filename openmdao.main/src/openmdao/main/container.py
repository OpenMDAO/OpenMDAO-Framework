...
from openmdao.main.macro import CommandRecorder
...
def recorded(fnc):
    ...
    If CommandRecorder.changes_are_recorded is False, this decorator does not decorate
    the function.
    """
    if CommandRecorder.changes_are_recorded:
        def _wrap(self, *args, **kwargs):
            CommandRecorder.record(fnc.__name__, self, args, kwargs)
...
def recorded_funct(fnc):
    ...
    if CommandRecorder.changes_are_recorded:
        def _wrap(*args, **kwargs):
            CommandRecorder.record(fnc.__name__, None, args, kwargs)
            ret = fnc(*args, **kwargs)
            CommandRecorder.pop()
            return ret
        return _wrap
