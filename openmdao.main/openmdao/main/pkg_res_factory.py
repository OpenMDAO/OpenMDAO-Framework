...
def import_version(req, env=None):
    ...
    # see if the requested distrib is available either in the current working_set or
    # in the environment
    try:
...
class EntryPtLoader(object):
    ...
    def create(self, env, name):
        """Return the object created by calling the entry point specified
        by typ. If necessary,
...
class PkgResourcesFactory(Factory):
    """A Factory that loads plugins using the pkg_resources API, which means it searches
    through egg info of distributions in order to find any entry point groups corresponding
    to openmdao plugin types, e.g., openmdao.components, openmdao.variables, etc.
    """
    ...
    def create(self, typ, name=None, version=None, server=None, 
        ...
        for entry in self._loaders[typ]:
            if entry.dist in Requirement.parse(entry.dist.project_name+'=='+version):
                return entry.create(self.env, name)

    
    def _get_plugin_info(self, pkg_env, groupname):
        """Given a search path and an entry point group name, fill the self._loaders dict with 
        EntryPtLoader objects. This only searches distributions and gathers information. 
            ...
            for dist in pkg_env[name]:
                ...
                for entry_pt in entry_dict.values():
                    if len(entry_pt.attrs) > 0:
                        ename = '.'.join([entry_pt.module_name]+list(entry_pt.attrs))
                    else:
                        raise NameError('entry point in setup.py file'+
                                        ' must specify an object within the module')
                    if ename not in self._loaders:
                        self._loaders[ename] = []
                    self._loaders[ename].append(EntryPtLoader(name=ename, group=groupname,
