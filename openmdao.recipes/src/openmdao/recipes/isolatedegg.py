...
class IsolatedEgg(zc.recipe.egg.Scripts):
    ...
    def __init__(self, buildout, name, options):
        self._bindir = buildout['buildout']['bin-directory']
        self._script_template = None
        self._py_script_template = None
        super(IsolatedEgg, self).__init__(buildout, name, options)

    def install(self):
        self._make_templates()
        old, old_py = _swap_templates(self._script_template, self._py_script_template)
        ...
        if sys.platform != 'win32':
            python = os.path.join(self._bindir, 'python')
            pythonVR = os.path.join(self._bindir, 'python'+sys.version[:3])
