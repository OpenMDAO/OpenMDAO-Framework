...
class FloatProxy(Float):
    """ Example of a proxy/property trait. """

    def __init__(self, **metadata):
        super(FloatProxy, self).__init__(**metadata)
        self._metadata['type'] = 'property'  # Just to show correct type.
        self._default = metadata.get('default_value', 0.)
        self._remote = {}

    def get(self, obj, name):
        if name not in obj._vars:
            obj._vars[name] = self._default
        return obj._vars[name]

    def set(self, obj, name, value):
        obj._vars[name] = value


class SimpleProxy(Component):
    """ Example of component that uses proxies to access internal data. """

    x = FloatProxy(iotype='in')
    f_x = FloatProxy(iotype='out')
...
class TestProxy(unittest.TestCase):
    """ Test MetaModel of proxied component. """
